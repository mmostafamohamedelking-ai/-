...
export default function BlogElement({image , readTime , date , category , title , excerpt , author:{name , avatar , role} , tags , slug}) {
  return (
   <>
   <article className="group card overflow-hidden" style={{ animationDelay: "0ms" }}>
                ...
                <Link className="block" to= {`/blog/${slug}`} >
                  <div className="relative h-52 overflow-hidden">
                    <img alt={slug} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out" src={image} />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    <div className="absolute top-4 right-4">
                      <span className="px-3 py-1 bg-[#0a0a0a]/80 backdrop-blur-sm text-white text-xs font-semibold rounded-full border border-[#333333]">{category}</span>
                    </div>
                  </div>
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-[#262626]">
                      <div className="flex items-center gap-3">
                        <img alt={name} className="w-9 h-9 rounded-full object-cover ring-2 ring-[#262626]" src={avatar} />
                        <div>

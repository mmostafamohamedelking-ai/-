...
export default function MyComponent() {
  ...
  const {title , excerpt , category , content , author: {name, avatar ,role} , image , date , readTime} = posts.find((el) => {
    return el.slug === slug;
  })

  const filtered = posts.filter((el) => {
    return el.category === category && el.slug !== slug;
  });
  const filteredElements = filtered.slice(0 , 3);
...
const { intro, sections } = parsePostContent(content);
      ...
      <div className="relative h-[60vh] min-h-[500px] overflow-hidden">
        ...
        <div className="absolute top-8 right-8 left-8">
          <nav className="inline-flex items-center gap-2 px-4 py-2 bg-black/30 backdrop-blur-md rounded-full text-sm border border-white/10">
            <a className="text-white/70 hover:text-white transition-colors" to="/" data-discover="true">
              <i className="fa-solid fa-home">
              </i>
            </a>
            <i className="fa-solid fa-chevron-left text-white/30 text-xs">
            </i>
            <Link className="text-white/70 hover:text-white transition-colors" to="/blog" data-discover="true">
              المدونة
            </Link>
            <i className="fa-solid fa-chevron-left text-white/30 text-xs">
            </i>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
          <div className="max-w-5xl mx-auto">
            <div className="flex flex-wrap items-center gap-3 mb-6">
              <Link className="px-4 py-2 bg-orange-500 text-white text-sm font-bold rounded-full hover:bg-orange-600 transition-colors" to={`/blog?category=${category}`} data-discover="true">
               {category}
              </Link>
            ...
            <div className="flex items-center gap-4 p-4 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 w-fit">
              <img alt="رامي الخطيب" className="w-14 h-14 rounded-full object-cover ring-2 ring-orange-500/50" src={avatar} />
              <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid lg:grid-cols-[1fr_300px] gap-12">
          <div className="order-2 lg:order-1">
            ...
            {sections.map((el , i) => (
              ...
              <h2 id={`section-${i}`} className="text-2xl md:text-3xl font-bold text-white mt-14 mb-6 flex items-center gap-4 scroll-mt-24">
            ...
            <div className="lg:sticky lg:top-24 space-y-6">
              <div className="p-6 bg-[#111111] rounded-2xl border border-[#262626]">
                ...
                <nav className="space-y-2">
                  {sections.map((el , i) => (
                     <a href={`#section-${i}`} className="flex items-center gap-3 p-3 rounded-xl text-neutral-400 hover:text-orange-500 hover:bg-orange-500/5 transition-all duration-300 group">
              ...
              <div className="p-6 bg-gradient-to-br from-orange-500/10 to-yellow-500/5 rounded-2xl border border-orange-500/20">
                <div className="text-center">
                  ...
                  <Link className="block w-full py-3 bg-orange-500 text-white font-semibold rounded-xl hover:bg-orange-600 transition-colors text-center" to="/blog" data-discover="true">
                    تصفح المزيد
                  </Link>
                </div>
              </div>
        ...
        <div className="mt-20 pt-12 border-t border-[#262626]">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link className="hidden sm:flex items-center gap-2 text-orange-500 hover:text-orange-400 transition-colors group" to="/blog" data-discover="true">
              عرض الكل
